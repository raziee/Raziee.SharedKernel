...
{
    private readonly Mock<ILogger<Raziee.SharedKernel.CQRS.LoggingBehavior<TestRequest, TestResponse>>> _loggerMock;
    private readonly Raziee.SharedKernel.CQRS.LoggingBehavior<TestRequest, TestResponse> _behavior;

    public LoggingBehaviorTests()
    {
        _loggerMock = new Mock<ILogger<Raziee.SharedKernel.CQRS.LoggingBehavior<TestRequest, TestResponse>>>();
        _behavior = new Raziee.SharedKernel.CQRS.LoggingBehavior<TestRequest, TestResponse>(_loggerMock.Object);
    }
    ...
    public async Task Handle_WithNullRequest_ShouldNotThrow()
    {
        ...
        // Act & Assert
        var action = async () => await _behavior.Handle(request!, next.Object, CancellationToken.None);
